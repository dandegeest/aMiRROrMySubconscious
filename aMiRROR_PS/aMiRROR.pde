...
void captureAndProcess() {
  ...
  println("Saved camera capture: " + captureFilename);
  ...
  try {
    ...
    if (statusCode == 200) {
      ...
      if (jsonResponse != null && jsonResponse.getBoolean("success")) {
        ...
        println("Got AI image URL: " + outputUrl);
...
void downloadImage(String url, String outputFilename) {
  try {
    ...
    if (!outputDir.exists()) {
      println("Creating output directory: " + outputDir.getAbsolutePath());
      outputDir.mkdirs();
    }
    
    // Get full path and ensure it's valid
    Path outputPath = Paths.get(dataPath(outputFilename));
    println("Saving to path: " + outputPath.toString());
    ...
    try {
      ...
      if (response.statusCode() == 200) {
        ...
        println("Saved AI image: " + outputFilename);
        ...
        if (newAIImage != null) {
          ...
          transitionAlpha = 0;
          println("Successfully loaded new AI image");
...
void displayStatus() {
  ...
  if (showSettings) {
    ...
    // Add camera preview at 1/4 scale
    if (currentCamImage != null) {
      // Calculate preview dimensions (1/4 of actual size)
      int previewWidth = currentCamImage.width / 6;
      int previewHeight = currentCamImage.height / 6;
    ...
    if (requestInProgress) {
      text("Generating... " + ((millis() - requestStartTime) / 1000) + "s", 20, 50);
    } else {
      text("Next capture in " + ((captureInterval - (millis() - lastCaptureTime)) / 1000) + "s", 20, 50);
    }
    
    text("Press TAB for settings", 20, 70);
  }
}
...
void keyPressed() {
  if (key == 's' || key == 'S') {
    ...
    currentPrompt = "A mushroom trip";
  } else if (key == '2') {
    currentPrompt = "A surrealist painting";
  } else if (key == '3') {
    currentPrompt = "A vibrant pop art portrait";
  } else if (key == '4') {
    currentPrompt = "An impressionist masterpiece";
  } else if (key == ' ') {
  ...
  if (keyCode == UP) {
    ...
    println("Guidance Scale: " + guidanceScale);
  } else if (keyCode == RIGHT) {
    guidanceScale = constrain(guidanceScale + 0.01, 0, 10);
    println("Guidance Scale: " + guidanceScale);
  }
  
  // Plus/minus for prompt strength
  if (key == '+' || key == '=') {
    promptStrength = constrain(promptStrength + 0.01, 0, 1);
    println("Prompt Strength: " + promptStrength);
  } else if (key == '-' || key == '_') {
    promptStrength = constrain(promptStrength - 0.01, 0, 1);
    println("Prompt Strength: " + promptStrength);
  }
